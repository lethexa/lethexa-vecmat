...
(function (exports) {
     ...
     * Checks for equality and returns true if equal
     * @method isEqualTo
     * @param v {Triangle} The triangle to check against.
     * @return True if both are equal 
     */ 
    ...
    exports.createBoundingPolygon = function(points) {
      ...
      var getAngleOfLine = function(p1, p2) {
        var diffVector = p2.sub(p1);
        return Math.atan2(diffVector.y(), diffVector.x());
      };
    
      var isOnLeftOfLineFromPoint = function(ptToCheck, p1, p2) {
        var a1 = toRangePI_PI(getAngleOfLine(p1, ptToCheck));
        var a2 = toRangePI_PI(getAngleOfLine(p1, p2));
        return toRangePI_PI(a1-a2) >= 0.0;
      };
      ...
      do {
        ...
        for(var j=0;j<points.length;j++) {
          ...
          if(pointOnHull !== point) {
            ...
            if((endPoint === pointOnHull) || isOnLeftOfLineFromPoint(point, pointOnHull, endPoint)) {
              if(arePointsEqual(pointOnHull, point))
                return;
              endPoint = point;
            }
          }
        ...
        i += 1;
      } while(!arePointsEqual(endPoint, result[0]));
