...
(function (exports) {
    ...
    exports.Sphere.prototype.intersect = function(ray) {
      ...
      var d1 = -x1 - root;
      var d2 = -x1 + root;
      var result = d1 < d2 ? d1 : d2; // Take the nearest...
...
//int pnpoly(int nvert, float *vertx, float *verty, float testx, float testy)
	...
	var c = false;
	var nvert = corners.length;
	for(var i=0,j=nvert-1;i<nvert;j=i++) {
	  if(((corners[i].y() > pt.y()) != (corners[j].y()>pt.y())) && (pt.x() < (corners[j].x()-corners[i].x) * (pt.y()-corners[i].y()) / (corners[j].y()-corners[i].y()) + corners[i].x()) )
	    c = !c;
	}
	return c;	
      };
